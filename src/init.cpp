...
#include "config/clonecoin-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "clonecoin.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "clonecoind.pid") + "\n";
#endif
    ...
    strUsage += "  -port=<port>           " + strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 59966, 59967) + "\n";
    ...
    strUsage += "                           clonecoin (or specifically: obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        strUsage += ", qt";
    strUsage += ".\n";
#ifdef ENABLE_WALLET
    ...
    strUsage += "  -litemode=<n>          " + strprintf(_("Disable all CCoin specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0) + "\n";
    ...
    strUsage += "  -masternodeaddr=<n>        " + strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:59966") + "\n";
    ...
    strUsage += "  -anonymizeclonecoinamount=<n>     " + strprintf(_("Keep N CCC anonymized (default: %u)"), 0) + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 56699, 56698) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The CCoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("clonecoin-loadblk");
...
/** Sanity checks
 *  Ensure that CCoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. CCoin Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CCoin Core is probably already running."), strDataDir));
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of CCoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart CCoin Core to complete") << "\n";
    ...
    nAnonymizeClonecoinAmount = GetArg("-anonymizeclonecoinamount", 0);
    if (nAnonymizeClonecoinAmount > 999999) nAnonymizeClonecoinAmount = 999999;
    if (nAnonymizeClonecoinAmount < 2) nAnonymizeClonecoinAmount = 2;
    ...
    LogPrintf("Anonymize CCoin Amount %d\n", nAnonymizeClonecoinAmount);
