...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeClonecoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeClonecoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeClonecoinAmount = strAnonymizeClonecoinAmount.remove(strAnonymizeClonecoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeClonecoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeClonecoinAmount * COIN) nMaxToAnonymize = nAnonymizeClonecoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeClonecoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeClonecoinAmount));
        strAnonymizeClonecoinAmount = strAnonymizeClonecoinAmount.remove(strAnonymizeClonecoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeClonecoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeClonecoinAmount == 0) {
